...
3. 以下のように、`RequestConfiguration.test_device_ids` 配列にテストデバイス ID を含めるようにコードを更新します。
...
メディエーション広告ネットワークのアダプタがテスト広告をサポートしているか不明な場合は、開発フェーズ中にそのネットワークの広告をクリックしないことをお勧めします。広告フォーマット内の [ResponseInfo.mediation_adapter_class_name](https://github.com/poingstudios/godot-admob-plugin/blob/master/addons/admob/src/api/core/ResponseInfo.gd) 属性を使用して、現在の広告を配信した広告ネットワークを特定できます。
