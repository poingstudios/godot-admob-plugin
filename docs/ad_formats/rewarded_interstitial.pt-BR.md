...
### Carregar um anúncio
Para carregar um anúncio intersticial premiado, utilize a classe `RewardedInterstitialAdLoader`. Passe um `RewardedInterstitialAdLoadCallback` para receber o anúncio carregado ou quaisquer erros potenciais. Vale a pena notar que, semelhante aos callbacks de carregamento de outros formatos, o `RewardedInterstitialAdLoadCallback` aproveita o `LoadAdError` para fornecer detalhes abrangentes do erro.
...
O `FullScreenContentCallback` gerencia eventos associados à exibição do seu `RewardedInterstitialAd`. Antes de apresentar o `RewardedInterstitialAd`, certifique-se de configurar o callback:
...
Ao apresentar um anúncio intersticial premiado, você usará um objeto `OnUserEarnedRewardListener` para gerenciar eventos relacionados a recompensas.
