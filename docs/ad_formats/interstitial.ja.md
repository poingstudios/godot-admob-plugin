...
### 広告のロード
インタースティシャル広告をロードするには、`InterstitialAdLoader` クラスを使用します。ロードされた広告またはエラーを受け取るために、`InterstitialAdLoadCallback` を渡します。他の広告フォーマットのロードコールバックと同様に、`InterstitialAdLoadCallback` は詳細なエラー情報を提供するために `LoadAdError` を利用します。
...
`FullScreenContentCallback` は、`InterstitialAd` の表示に関連するイベントを管理します。`InterstitialAd` を表示する前に、必ずコールバックを設定してください。
