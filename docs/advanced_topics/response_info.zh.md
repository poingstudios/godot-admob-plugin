...
出于调试和记录日志的目的，成功加载的广告会提供一个 [`ResponseInfo`](../reference/classes/ResponseInfo.md) 对象。此对象包含已加载广告的信息，以及用于加载广告的中介瀑布流（mediation waterfall）信息。

在广告成功加载并且展示已付费的情况下，可以通过 `on_ad_paid` 回调返回的 `AdValue.response_info`（C# 中为 `AdValue.ResponseInfo`）属性来获取响应信息。此外，特别是针对 [`AppOpenAd`](../reference/classes/AppOpenAd.md)，您可以通过 `get_response_info()`（C# 中为 `GetResponseInfo()`）方法来检索它。

在广告加载失败且仅有错误可用的情况下，可以通过 [`LoadAdError`](../reference/classes/LoadAdError.md).response_info（C# 中为 `LoadAdError.ResponseInfo`）获取响应信息。
...
[`ResponseInfo`](../reference/classes/ResponseInfo.md) 对象上的属性包括：

| 属性 | 描述 |
| -------- | ----------- |
| `adapter_responses` (`AdapterResponses`) | 返回 [`AdapterResponseInfo`](../reference/classes/AdapterResponseInfo.md) 列表，其中包含广告响应中包含的每个适配器的元数据。可用于调试瀑布流中介和竞价的执行情况。列表的顺序与此广告请求的中介瀑布流顺序相匹配。有关更多信息，请参阅[适配器响应信息](#adapter-response-info)。 |
| `loaded_adapter_response_info` (`LoadedAdapterResponseInfo`) | 返回加载了广告的适配器所对应的 [`AdapterResponseInfo`](../reference/classes/AdapterResponseInfo.md)。 |
...
[`AdapterResponseInfo`](../reference/classes/AdapterResponseInfo.md) 包含广告响应中单个广告源的响应信息。
...
对于每个广告源，[`AdapterResponseInfo`](../reference/classes/AdapterResponseInfo.md) 提供以下属性：

| 属性 | 描述 |
| -------- | ----------- |
| `ad_error` ([`AdError`](../reference/classes/AdError.md)) | 获取与向该广告源发起请求相关的错误。如果广告源成功加载了广告或未尝试请求该广告源，则返回 `null`。 |
