...
モック広告を使用するために、GDScript や C# のコード、または設定を変更する必要はありません。すべての標準的な [`MobileAds`](reference/classes/MobileAds.md) および広告フォーマットの API は、まったく同じように機能します。
...
- カスタム画面位置（[`AdPosition`](reference/classes/AdPosition.md).custom(x, y)）をサポートします。
...
- モックテンプレートが配置されると `on_native_overlay_ad_rendered` シグナルを発行し、実際のデバイス動作をミラーリングします。これにより、広告が完全にレイアウトされた後にのみ UI（`SafeArea` など）を安全に調整できます。
