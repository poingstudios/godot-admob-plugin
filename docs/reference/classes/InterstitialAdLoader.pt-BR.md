...
A classe `InterstitialAdLoader` é responsável por executar solicitações de anúncio assíncronas para carregar instâncias de [`InterstitialAd`](InterstitialAd.md).

## Métodos

### `load` / `Load`

Envia uma solicitação de carregamento para um anúncio Interstitial.
