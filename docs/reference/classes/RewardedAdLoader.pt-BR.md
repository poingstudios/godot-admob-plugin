...
A classe `RewardedAdLoader` é responsável por executar solicitações de anúncio assíncronas para carregar instâncias de [`RewardedAd`](RewardedAd.md).

## Métodos

### `load` / `Load`

Envia uma solicitação de carregamento para um anúncio Recompensado.
