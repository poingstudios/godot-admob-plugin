...
A classe `AppOpenAdLoader` é responsável por executar solicitações de anúncio assíncronas para carregar instâncias de [`AppOpenAd`](AppOpenAd.md).

## Métodos

### `load` / `Load`

Envia uma solicitação de carregamento para um anúncio App Open.
