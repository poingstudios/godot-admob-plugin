...
A classe `RewardedInterstitialAdLoader` é responsável por executar solicitações de anúncio assíncronas para carregar instâncias de [`RewardedInterstitialAd`](RewardedInterstitialAd.md).

## Métodos

### `load` / `Load`

Envia uma solicitação de carregamento para um anúncio Interstitial Recompensado.
