...
=== "iOS"
    ...
    1. 导出项目时，使用您的 [AdMob App ID](https://support.google.com/admob/answer/7356431) 更新 `GADApplicationIdentifier`，并确保在导出对话框的 Plugins 部分中启用了 `Ad Mob`。如果您有中介，也要勾选 `Ad Mob Meta` 等...
    
    1. **这就完成了！** 由于此插件使用 `.xcframework` 包，Godot 4.2+ 将自动将所有必需的库和框架集成到您的 Xcode 项目中。不需要手动的终端命令、CocoaPods 或 Xcode 配置步骤。
    
    1. [如果您面临 "__swift_FORCE_LOAD_" 错误，请阅读此文](https://github.com/poingstudios/godot-admob-ios/issues/127)。
 
        1. 在您的 Xcode 项目中创建一个 `Untitled.swift` 文件。
        2. Xcode 会询问您是否 `Create Bridge Header`，选择 `Accept`。
        3. 您的项目现在应该可以正常构建了。
...
在加载广告之前，请确保您的应用初始化了 Google 移动广告 SDK。您可以通过调用 `MobileAds.initialize()` 来实现这一点。此函数会初始化 SDK，并在初始化过程完成或超过 30 秒超时后触发完成监听器。请注意，此初始化应仅执行一次，最好是在应用启动阶段。
