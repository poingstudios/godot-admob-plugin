...
将 AdMob 插件集成到您的 Godot 项目中（特别适用于 Godot v4.4+）是启用广告展示和产生收入的首要且关键的步骤。成功加入此插件后，您可以灵活地从各种广告格式（如横幅或插屏）中进行选择，并继续执行必要的实现步骤。
...
!!! info "信息"
    本节通常 **不是必需的**，因为插件会自动处理库。只有在自动下载失败或更新 Godot 版本时才需要按照以下步骤操作。
