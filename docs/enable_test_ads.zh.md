...
3. 更新您的代码，将测试设备 ID 包含在您的 [`RequestConfiguration`](reference/classes/RequestConfiguration.md).test_device_ids 数组中，如下所示：
...
如果您不确定某个中介广告网络适配器是否支持测试广告，建议在开发阶段避免点击来自该网络的广告。您可以使用任何广告格式中的 [`ResponseInfo.mediation_adapter_class_name`](reference/classes/ResponseInfo.md#mediation_adapter_class_name) 属性来确定哪个广告网络投放了当前的广告。
